...
$GLOBALS['_transient']['static']['nusoap_base_two']['globalDebugLevel'] = 9;





class nusoap_client_two extends nusoap_base_two  {
	...
	* @access   public
	*/
	function nusoap_client_two($endpoint,$wsdl = false,$proxyhost = false,$proxyport = false,$proxyusername = false, $proxypassword = false, $timeout = 0, $response_timeout = 30, $portName = ''){
		parent::nusoap_base_two();
	...
	*/
	...
	function loadWSDL() {
		...
		$this->wsdl = new wsdl_two('',$this->proxyhost,$this->proxyport,$this->proxyusername,$this->proxypassword,$this->timeout,$this->response_timeout,$this->curl_options,$this->use_curl);
		...
		switch(true){
			...
			case preg_match('/^http/',$this->endpoint):
				...
				if($this->persistentConnection == true && is_object($this->persistentConnection)){
					...
					$http = new soap_transport_http_two($this->endpoint, $this->curl_options, $this->use_curl);
	...
	*/
    function parseResponse($headers, $data) {
		...
		if (!strstr($headers['content-type'], 'application/soap+xml')) {
			$this->setError('Response not of type tapplication/soap+xml: ' . $headers['content-type']);
			return false;
		}
		...
		$parser = new nusoap_parser_two($data,$this->xml_encoding,$this->operation,$this->decode_utf8);
...
class nusoap_base_two {
	...
	* @access	public
	*/
	function nusoap_base_two() {
		$this->debugLevel = $GLOBALS['_transient']['static']['nusoap_base_two']['globalDebugLevel'];
	}
	...
	*/
	function getGlobalDebugLevel() {
		return $GLOBALS['_transient']['static']['nusoap_base_two']['globalDebugLevel'];
	}
	...
	*/
	function setGlobalDebugLevel($level) {
		$GLOBALS['_transient']['static']['nusoap_base_two']['globalDebugLevel'] = $level;
	}
	...
	*Here is the same…but the culture here is mixed, prim
...
if (!extension_loaded('soap')) {
	 ...
	 */
	class soapclient extends nusoap_client_two {
	}
}
...
class soap_transport_http_two extends nusoap_base_two {
	...
	* @access public
	*/
	function soap_transport_http_two($url, $curl_options = NULL, $use_curl = false){
		parent::nusoap_base_two();
...
* @access public 
*/
class wsdl_two extends nusoap_base_two {
     ...
     * @access public 
     */
    function wsdl_two($wsdl = '',$proxyhost=false,$proxyport=false,$proxyusername=false,$proxypassword=false,$timeout=0,$response_timeout=30,$curl_options=null,$use_curl=false){
		parent::nusoap_base_two();
		$this->debug("ctor wsdl=$wsdl timeout=$timeout response_timeout=$response_timeout");
        $this->proxyhost = $proxyhost;
        $this->proxyport = $proxyport;
    ...
    function parseWSDL($wsdl = '') {
        ...
        if (isset($wsdl_props['scheme']) && ($wsdl_props['scheme'] == 'http' || $wsdl_props['scheme'] == 'https')) {
        	...
        	// get wsdl
	        $tr = new soap_transport_http_two($wsdl, $this->curl_options, $this->use_curl);
    ...
    function start_element($parser, $name, $attrs)
    {
        if ($this->status == 'schema') {
            ...
            $this->status = 'schema';
            $this->currentSchema = new nusoap_xmlschema_two('', '', $this->namespaces);
            $this->currentSchema->schemaStartElement($parser, $name, $attrs);
            $this->appendDebug($this->currentSchema->getDebug());
            $this->currentSchema->clearDebug();
        } else {
...
* @access   public
*/
class nusoap_xmlschema_two extends nusoap_base_two  {
	...
	* @access   public
	*/
	function nusoap_xmlschema_two($schema='',$xml='',$namespaces=array()){
		parent::nusoap_base_two();
...
* @access   public
*/
class nusoap_parser_two extends nusoap_base_two {
	...
	* @access   public
	*/
	function nusoap_parser_two($xml,$encoding='UTF-8',$method='',$decode_utf8=true){
		parent::nusoap_base_two();
		$this->xml = $xml;
